...
if not os.path.exists('../saved_model/' + os.path.basename(__file__)[:-3] + '_' + datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')):
    os.mkdir('../saved_model/' + os.path.basename(__file__)[:-3] + '_' + datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
if not os.path.exists(param_save_prefix):
    os.mkdir(param_save_prefix)

# the frequency of model saving,
# namely saving the model params every param_model_save_interval loops
param_model_save_interval = 500
