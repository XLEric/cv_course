...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black',
    ...
    if dataset == 'coco_hp':
      ...
      self.ec = [(255, 0, 0), (0, 255, 0), (255, 50, 50), (0, 255, 15),
                 (255, 0, 0), (0, 255, 35), (25, 120, 255),
                 (255, 0, 100), (255, 0, 100), (255, 255, 55), (255, 255, 75),
                 (255, 80, 70), (120, 120, 255), (55, 130, 255),
                 (255, 120, 0), (255, 120, 0), (255, 0, 255), (255, 0, 255)]
      self.colors_hp = [(255, 255, 0), (255, 0, 0), (0, 0, 255),
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'):
    ...
    # txt = '{}{:.2f}'.format('person : ', conf)
    txt = '{}'.format('person')
    font = cv2.FONT_HERSHEY_SIMPLEX
    cat_size = cv2.getTextSize(txt, font, 0.9, 2)[0]
    cv2.rectangle(
      self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0,0,255), 2)
    if show_txt:
      cv2.rectangle(self.imgs[img_id],
                    (bbox[0], bbox[1] - cat_size[1] - 2),
                    (bbox[0] + cat_size[0], bbox[1] - 2), c, -1)
      cv2.putText(self.imgs[img_id], txt, (bbox[0], bbox[1] - 2),
                  font, 0.9, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
  ...
  def add_coco_hp(self, points, img_id='default'):
    ...
    for j in range(self.num_joints):
      ...
      if j ==0 or j ==1 or j ==2 or j ==3 or j==4:
          cv2.circle(self.imgs[img_id],(points[j, 0], points[j, 1]), 6,(255,0,255), -1)
          face_pts.append([points[j, 0], points[j, 1]])

    for j, e in enumerate(self.edges):
      if points[e].min() > 0:
        cv2.line(self.imgs[img_id], (points[e[0], 0], points[e[0], 1]),
                      (points[e[1], 0], points[e[1], 1]), self.ec[j], 4,
    ...
    if not self.ipynb:
      ...
      for i, (k, v) in enumerate(self.imgs.items()):
        ...
        if len(v.shape) == 3:
          ...
          # cv2.namedWindow('multi_pose',0)
          # cv2.imshow('multi_pose',v)
          # cv2.waitKey(0)
        else:
          cv2.namedWindow('multi_posexx',0)
          cv2.imshow('multi_posexx',v)
          cv2.waitKey(0)
          self.plt.imshow(v)
      self.plt.show()
