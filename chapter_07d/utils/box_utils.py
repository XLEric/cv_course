...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    """
    # jaccard index,计算 gt bbox 与 priors iou，gt bbox 与 priors 都为 归一化全局坐标
    overlaps = jaccard(
        truths,
        point_form(priors)# 将 priors (x,y,w,h)转换为(x1,y1,x2,y2)格式
    )
    # (Bipartite Matching)
    ...
    确保匹配的框不会因为阈值太低被过滤掉
    best_truth是每一个priors box匹配一个最好的ground truth
    """
    ...
    前面的,无法解决两个物体匹配到同一个default box的问题
    """
...
def log_sum_exp(x):
    ...
    """
    x_max = x.data.max()
    # torch.exp(x-x_max)，取指数时的最大值为0.使得绝对不会出现上溢(overflow)，即使其余的下溢(underflow)
