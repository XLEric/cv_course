...
class Detect(Function):
    ...
    def __init__(self, num_classes, bkg_label, cfg):
        self.num_classes = num_classes # 类别数 ： 背景 + 目标
        self.background_label = bkg_label # 背景的标签
    ...
    def forward(self, predictions, prior):
        ...
        # print('Detect',loc.size(), conf.size())
        ...
        for i in range(num):# prior_data ： 针对 不同的 feature map ，全图 归一化的中心坐标
            decoded_boxes = decode(loc_data[i], prior_data, self.variance)
            # conf_scores = conf_preds[i].clone()
            # conf_scores = torch.tensor(conf_preds[i])
            conf_scores = conf_preds[i].clone().detach()
